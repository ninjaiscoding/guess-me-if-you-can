...
def assign_custom_words(players: List[Dict[str, Any]]):
    n = len(players)
    if n == 0:
        return
    if n == 1:
        players[0]["character"] = players[0]["customWordGiven"]
        return

    given_words = [p["customWordGiven"] for p in players]
    best_assignment = None
    min_self_matches = n + 1
    
    for _ in range(1000):
        candidate = given_words.copy()
        random.shuffle(candidate)
        matches = sum(1 for i in range(n) if candidate[i] == given_words[i])
        if matches == 0:
            best_assignment = candidate
            break
        if matches < min_self_matches:
            min_self_matches = matches
            best_assignment = candidate

    if sum(1 for i in range(n) if best_assignment[i] == given_words[i]) > 0:
        for i in range(n):
            if best_assignment[i] == given_words[i]:
                swapped = False
                for j in range(n):
                    if i != j:
                        if (best_assignment[j] != given_words[i]) and (best_assignment[i] != given_words[j]):
                            best_assignment[i], best_assignment[j] = best_assignment[j], best_assignment[i]
                            swapped = True
                            break
                if not swapped:
                    for j in range(n):
                        if i != j and best_assignment[i] != best_assignment[j]:
                            best_assignment[i], best_assignment[j] = best_assignment[j], best_assignment[i]
                            break

    for i, p in enumerate(players):
        p["character"] = best_assignment[i]
            ...
            if action == "join":
                ...
                if current_room and rooms[current_room]["hostId"] == player_id:
                    ...
                    if target_player:
                        ...
                        # 3. Handle game progression updates if they were kicked during play or word submission
                        if room["phase"] == "playing":
                            if room["turnIndex"] >= len(room["players"]):
                                room["turnIndex"] = 0
                            check_game_over(current_room)
                        elif room["phase"] == "custom_words_assign":
                            if len(room["players"]) > 0 and all(p.get("customWordGiven", "") != "" for p in room["players"]):
                                assign_custom_words(room["players"])
                                for p in room["players"]:
                                    p["guessesLeft"] = 3
                                    p["isWinner"] = False
                                    p["isLoser"] = False
                ...
                if current_room and rooms[current_room]["hostId"] == player_id:
                    ...
                    if room["customWordsEnabled"]:
                        ...
                        if "usedWords" not in room:
                            room["usedWords"] = {}
                        if cat not in room["usedWords"]:
                            room["usedWords"][cat] = set()
                            
                        used_set = room["usedWords"][cat]
                        available_chars = [c for c in chars if c not in used_set]
                        num_players = len(room["players"])
                        
                        if len(available_chars) < num_players:
                            used_set.clear()
                        ...
                        for i, p in enumerate(room["players"]):
                            chosen_char = available_chars[i % len(available_chars)]
                            p["character"] = chosen_char
                            used_set.add(chosen_char)
                            p["guessesLeft"] = 3
                            p["isWinner"] = False
                            p["isLoser"] = False
